...
async def cmd_start(message: Message, state: FSMContext) -> None:
    ...
    if user and await db.is_setup_complete(message.from_user.id):
        await state.clear()
...
async def setup_level_chosen(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await db.update_user(callback.from_user.id, prog_language=prog_language, level=level)
...
async def send_next_problem(
    ...
    problem = await db.get_next_problem(user_id, level)
    if not problem:
    ...
    record_id = await db.record_problem_sent(user_id, problem["id"])
...
async def on_hint(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    problem = await db.get_problem(data["problem_id"])
    ...
    await db.update_problem_record(data["record_id"], hints_used=hints_revealed)
...
async def on_surrender(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await db.update_problem_record(data["record_id"], status="failed", solution_shown=1)
...
async def on_solution(message: Message, state: FSMContext) -> None:
    ...
    if is_correct:
        await db.update_problem_record(data["record_id"], status="solved")
        ...
        await db.update_problem_record(data["record_id"], status="failed")
...
async def on_show_solution(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    quote = get_quote(lang)
    await db.update_problem_record(data["record_id"], solution_shown=1)
...
async def on_set_prog_lang(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await db.update_user(callback.from_user.id, prog_language=lang_choice)
    user = await db.get_user(callback.from_user.id)
    lang = user.get("bot_language", "ru")
    await state.set_state(SettingsStates.main)
...
async def on_set_bot_lang(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await db.update_user(callback.from_user.id, bot_language=bot_lang)
    await state.set_state(SettingsStates.main)
    if bot_lang == "en":
...
async def on_set_level(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await db.update_user(callback.from_user.id, level=new_level)
    user = await db.get_user(callback.from_user.id)
    lang = user.get("bot_language", "ru")
...
async def on_set_time(message: Message, state: FSMContext) -> None:
    ...
    await db.update_user(message.from_user.id, notification_time=time_str)
    await state.set_state(SettingsStates.main)
