...
WELCOME_TEXT = (
    "🇬🇧 <b>English Conversation Practice</b>\n"
    "\n"
    "Практикуй разговорный английский с ИИ-собеседником!\n"
    "\n"
    "Как это работает:\n"
    "1. Нажми <b>New dialogue</b> — получишь тему и словарь\n"
    "2. Нажми <b>Start</b> — бот начнёт разговор голосом\n"
    "3. Отвечай голосовыми сообщениями на английском\n"
...
HELP_TEXT = (
    ...
    "  1. Нажми «New dialogue» в меню\n"
    "  2. Посмотри тему и словарь, нажми «Start»\n"
    "  3. Бот начнёт разговор голосом — отвечай голосовыми\n"
    "  4. После всех обменов или нажатия «End» — оценка\n"
    "\n"
    "<b>Уровни:</b>\n"
...
async def cmd_mystats(message: Message) -> None:
    ...
    if stats is None:
        await message.answer(
            ...
            "Нажми «New dialogue», чтобы начать!",
            parse_mode=ParseMode.HTML,
        )
        return
...
async def _generate_and_show_scenario(message: Message, state: FSMContext) -> None:
    ...
    topic = scenario_data.get("topic", "Conversation")
    scenario_text = scenario_data.get("scenario", "")
    vocabulary = scenario_data.get("vocabulary", [])
    opening_line = scenario_data.get("opening_line", "Hello!")
...
async def handle_text_during_conversation(message: Message) -> None:
    ...
    await message.answer(
        "🎤 Отправь <b>голосовое сообщение</b> на английском.",
        parse_mode=ParseMode.HTML,
    )
...
async def handle_unexpected_voice(message: Message, state: FSMContext) -> None:
    ...
    if current is None:
        ...
        await message.answer(
            ...
            "Нажми «New dialogue» для начала диалога.",
        )
...
async def main() -> None:
    logger.info("Starting English Conversation Practice bot...")
