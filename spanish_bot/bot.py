...
_log_handler = RotatingFileHandler("logs/spanish_bot.log", maxBytes=5_000_000, backupCount=3)
...
async def handle_start_conversation(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    try:
        ...
        await callback.message.answer(
            "⚠️ Не удалось озвучить сообщение. Попробуй нажать «Terminar» и начать заново.",
            parse_mode=ParseMode.HTML,
            reply_markup=conversation_keyboard(),
        )
